...
// Question 1: Chocolates and Wrappers
int question1countRec(int choc, int wrap)
{
    if (choc < wrap) return 0;
    int newChoc = choc/wrap;
    return newChoc + question1countRec(newChoc + choc%wrap,
                              wrap);
}
...
int countMaxChoco(int money, int price, int wrap)
{
    ...
    return choc + question1countRec(choc, wrap);
}
...
// The Best Way to Split a Lovely String
...
int chopTheTree()
{
    ...
    { // Test whether number of trees is larger than 0 and also integer test
        cout << "You must specify a positive integer number for the number of trees!" << endl;
        return -1;
    }

    map<string, float> treeTypesMap; // Mapping for DBH
...
void findVocabulary()
{
    // accept input
    string paragraph;
    cout << "Enter the paragraph you want to calculate for vocab: " << endl;
...
void DNAMatching()
{
    ...
    for (int i = 0; i < suspect_count; i++)
    {
        ...
        {
            cout << i + 1 << endl << longestSTRInCrimeScene[1] / STR.length() << endl; return;
        }
    }
    cout << 0 << endl
              << 0 << endl;
...
void twoOPT()
{
    ...
    {
        string swap;
        cout << "Please enter the start and end node of iteration " << i << ": ";
        cin >> swap;
...
void linearInterpolation()
{
    ...
    {
        string inp;
        cout << "Please enter the value for input #" << i + 1 << ": ";
        cin >> inp;
        input.push_back(inp);
    }
